...
Varyings LitPassVertex(Attributes input)
{
    ...
    // Displacement 
    half3 displacement = _ParallaxMap.SampleLevel(sampler_ParallaxMap, input.texcoord.xy, 0).rgb;
    half3 normal = _BumpMap.SampleLevel(sampler_BumpMap, input.texcoord.xy, 0).rgb;
    displacement = (displacement - 0.5) * _HeightWeight / 10;

    input.positionOS.xyz += input.normalOS.xyz * normal.xyz * displacement;
    //input.positionOS.xyz += normal.xyz * displacement;
...
#define DomainPos(fieldName) output.fieldName = \
        ...
        output.positionOS.xyz = lerp(output.positionOS.xyz, displacedPosition, _TessSmooth);
